...
% This was included alread, to be removed
...
%         problem.class(u).iat = problem.iat;  % Deterministic (constant) inter-arrival times (iat)
        problem.class(u).deadline = deadlineApps(u);  % Deadline in ms
        problem.class(u).name = problem.appNameInteretList{u};
        problem.class(u).numPkts = problem.numPkts;  % Number of packets for the class
        problem.class(u).iat = iatApps(u);  % Deterministic (constant) inter-arrival times (iat)
